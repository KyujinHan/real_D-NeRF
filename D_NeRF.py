# -*- coding: utf-8 -*-
"""
Created on Sat Dec 24 04:16:15 2022
...
from D_NeRF_function import *
from D_NeRF_model import NeRF
...
#############################################
# Fifth, get rays function
def get_rays(H, W, focal, c2w):
    #fx,fy,cx,cy = focal
    i, j = torch.meshgrid(torch.linspace(0, W-1, W), torch.linspace(0, H-1, H))  # pytorch's meshgrid has indexing='ij'
    i = i.t()
    j = j.t()
    dirs = torch.stack([(i-0.5*W)/focal, -(j-0.5*H)/focal, -torch.ones_like(i)], -1) # Cx = 0.5*w, Cy = 0.5*H
...
def train(args):
    ...
    if args.dataset_type == 'real':
        ...
        images, poses, times, render_poses, render_times, hwf, i_split = load_custom_data(args.datadir, args.half_res, args.testskip)
        print('Loaded real data', images.shape, render_poses.shape, hwf, args.datadir)
        i_train = i_split[0]
        #i_test = i_train
        #i_val = i_train
        ...
        1. Check near & far
        2. ndc right?
        3. colmap images sorting? - sol
        4. how to set the number of frames?
        
        # 2022/12/31
        -> ndc: False, near: 2, far: 6
        '''
        ...
        raise Exception("Unknown datasets.")
    ...
    if args.render_test:
        ...
        render_times = np.array(times[i_train])
